export default class Mesto {
  ...
  _getTemplate() {
    const mestoElement = document
      ...
      .content.querySelector(".mesto__item")
      .cloneNode(true);
  ...
  generateMesto() {
    ...
    this._mestoDeleteButton = this._element.querySelector(
      ".mesto__delete-button"
    ); // переменная для кнопки удаления
    this._mestoLikeCounter = this._element.querySelector(
      ".mesto__like-counter"
    ); // параграф для количества лайков
  ...
  _toggleLike() {
    if (this._isLiked()) {
      // если лайк есть, то коллбэк на удаление лайка
      this._handleDeleteLike(this._cardId);
    } else {
      // если нет, то на постановку лайка
